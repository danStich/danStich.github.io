...
The purpose of this page is to provide a general overview of the Merrimack River dam passage performance standard model for American shad. If you are looking for examples, please check <a href="examples.html">here</a>.

<h3 id="multi"><b>Habitat definitions</b></h3>

The Merrimack River Model includes American shad habitat from the mouth of the river upstream of Hookset Dam in the main-stem of the river. The model currently incorporates upstream and downstream migration through an alternative migration route, a bypass, at Pawtucket Dam in the lower main-stem of the river. Probability of using the bypass is input by the user for upstream and downstream migration as a decision point in the model.
...
Historical temperature data are compiled from the U.S. Geological gauge at South Branch Piscataqoug River near Goffstown, NH (ID 01091000) 2012-2018. These data are used to simulate new annual temperatures using the `simTemperature()` function in `shadia`, for example:

```{r}
# Simulate new temperatures for a single year
  newT <- shadia::simTemperature(shadia::tempData_merrimack)
  plot(newT,

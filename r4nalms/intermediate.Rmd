...
We will fit a model testing the effect of `Elevation` on `logSecchi`. But, first we'll drop rows that don't have `Latitude` information like we did when we were mapping.
...
Wow, this graph is almost as underwhelming as the R^2^ for the model that was used to create it. In fact, it looks like we're underpredicting basically all of the extremely high values of `Secchi`. **This should highlight the importance of communicating effects**. If we had just reported p-values, this would have gone un-noticed and someone might have misused the result.

...
The first line of the output gives us the actual data that with which we are working- nothing of interest here other than a quick sanity check.

The second line shows the 'statistics' that we are interested in:  _t_, the test statistic for the t-test. 
...
Let's keep working with the shad data for now for the sake of consistency. But, now we want to know if there is a difference in mean length of males ('bucks') and females ('roes'). This is a common type of boring question that we might ask with univariate tests (I didn't say that).
...
Now that we are using two samples, we should be cognizant that this test assumes equal variances unless we explicitly account for it as we did
above. Let's test it:
...
# We'll make our variables into objects so we don't have to type as much
  x = shad$Length[shad$Sex=='R']  # Group 1
  y = shad$Length[shad$Sex=='B']  # Group 2
# Now we will test the null hypothesis that there is no difference in
# variance between groups using an F test. To do this, the F test takes
...
SO...it's a good thing we assumed that the variances were unequal!
...
If we were in violation of normality, we would use the Wilcox test to
test for shifts in the central tendancy here. I will not go through
the whole thing again here, but I will note that the syntax is very much
the same to that of the t-test now:

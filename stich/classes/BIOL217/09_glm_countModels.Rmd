...
Let's use a crab data set to demonstrate the GLM with Poisson data, we will walk through this data set for both the Poisson and negative binomial examples, addressing some distributional assumptions and model fit along the
way.
...
These data represent the number of satellite male crabs per female (rows) horseshoe crab in relation to a number of characteristics of the females, including their color, spine condition, carapace width, and mass.
...
```{r}
hist(crabs$satellites, col='gray87', main='', ylab='', xlab='Number of satellites', yaxt='n')
```    
...
For this example, we will use a function called `glm.nb`. This function lets us estimate parameters for a GLM that uses the negative binomial error distribution and estimates the "overdispersion parameter" for the negative binomial distribution. You surely will remember this parameter and know it as `theta` from our discussions about probability distributions.
...
Play around with the two formulations above and see if there's a difference. **Hint**: there's not really. Just two different ways to do the same thing. The functionality in the `glm()` function only came around recently, that's all.

Now, let's take a look at the distribution of the residuals from the object we fit with `glm.nb`.
...
Luckily, there is also a tool for this purpose in the `boot` package.
...
The interpretation of this output is virtually identical to the output for linear models and other GLMs that we have been working with. Here, we see that no variable is significant with a Type-I error rate ($\alpha$) of 0.05, and that only the mass of females has a significant effect on the number of satellite males when we increase our Type-I error rate to 0.10. How does this compare to the original findings? Go take a look...

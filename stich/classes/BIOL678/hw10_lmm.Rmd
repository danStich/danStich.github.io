...
This week we will play with Yellow Perch data from the New York State Department of Environmental Conservation (NYSDEC) fisheries database. These data were collected by the NYSDEC throughout NY over the past few decades. 
...
# We are reading it in with the optional argument because we don't want to deal
# with code relicts when we start dropping factor levels
  fish = read.csv("http://employees.oneonta.edu/stichds/data/allwaters.txt", stringsAsFactors = FALSE)
...
As you can see, there are a *ton* of lakes in this dataset, and it contains information for both yellow perch (*Perca flavescens*) and walleye (*Sander vitreus*). The lakes are scattered throughout NY. So, before we can start working with the data we want, we need to get them out of this mess.
...
And that's the sad reality. In any case, this is more than enough for what we will do this week. 
...
\item 3. Graph the mean overall relationship between length and mass based on the overall intercept and slope.

\item 4. Determine whether any of the $\beta$s overlap with zero.

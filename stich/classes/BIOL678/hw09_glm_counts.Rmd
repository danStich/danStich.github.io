...
And, finally, we assume that the i^th^ value of $\alpha$ is drawn from a normal distribution on the link scale, dependent upon the color (j) of the female crab in the i^th^ observation. We specify the prior on alpha for each j^th^ color as a diffuse, normal distribution with a mean ($\mu$) of zero and a variance ($\sigma^2$) of 100 (in BUGS language $\tau = 0.01$).
...
> Define a function that creates a list of initial
...
## Shifting gears
This example is probably a good case for switching over to the negative binomial distribution. The code to demonstrate this is below. We drop the interaction term and focus on the big picture here. This is the formulation you will work with for the rest of lab.
...
modelstring="                 
  model {                       
    # Likelihood
      for(i in 1:nobs){    
        # Count is outcome of Negative binomial PDF
        N[i] ~ dnegbin(p[i], r) 
        
        # Re-define p (success parameter)
        # in terms of lambda (count) and
        # r (overdispersion)
        p[i] <- r/(r+lambda[i]) 
        
        # Log-link between lambda and linear
        # predictor
        log(lambda[i]) <- mu[i]
        
        # Expectation from linear predictor on log
        # scale
        mu[i] <- alpha[color[i]] + beta*X[i]
      }

    # Slope
      beta ~ dnorm(0,0.0001)
      r ~ dgamma(0.0001, 0.0001)T(0,3)
...
# Call jags and run the model
  crabs.model3 <- jags(crabs.data, inits=inits, parameters, 
...
We see that mass has a significant effect on number of satellites across color groups based on exclusion of zero from the 95% CRI. We can tell that the number of satellite males increases with increasing mass of females.

If we wanted to vizualize this, we could do something like:
```{r}
# Make new masses for prediction
X = seq(min(crabs.data$X), max(crabs.data$X), 0.2)

# Get posteriors
alpha = crabs.model3$BUGSoutput$sims.list$alpha
beta = crabs.model3$BUGSoutput$sims.list$beta

# Get posterior prediction for each iteration
# Choose a crab color group to predict. Change it out
# and see how the plots respond. The rest of the code
# stays the same when we do this.
color = 1

# Loop through manual predictions. Note we invert
# the log-link function with exp() inside this loop
pred = matrix(0, nrow = length(beta), ncol = length(X))
for(i in 1:length(beta)){
  for(t in 1:length(X)){
    pred[i, t] = exp(alpha[i, color] + beta[i] * X[t])
  }
}

# Calculate mean and 95% CRI of predicted satellites
# for each value in our new sequence of X used for preds
meanpred = apply(pred, 2, mean)
lwr = apply(pred, 2, quantile, probs=c(0.025))
upr = apply(pred, 2, quantile, probs=c(0.975))

# Make a white plot to start
plot(x = crabs$mass[crabs$color==color],
     y = crabs$satellites[crabs$color==color], yaxt='n',
     pch = 21, col = 'white',
     ylim = c(0, 20),
     ylab = 'Number of satellites', xlab = 'Mass',
     main = paste("Color group ", color, sep='')
     )

# Add a polygon for 95% CRI
polygon(x = c(X, rev(X)), y = c(lwr, rev(upr)),
        col = 'gray87', border = NA
        )

# Add the raw data as points
points(x = crabs$mass[crabs$color==color],
       y = crabs$satellites[crabs$color==color],
       pch = 21, col = 'black', bg = 'black')

# Add line for mean prediction
lines(x = X, y = meanpred, col='black', lty=1)

# Add the y-axis with rotated tick labels
axis(side = 2, las = 2)

# Drop a box around the plot
box()

```

This still isn't super satisfying because of that one data point on the far right that we saw back in diagnostics (try `color = 2` in code above), but the pattern generally fits the data. Of course, we would want to go back through and check it out, along with Bayesian p-values and other diagnostics.
...
In addition to these tasks, I would like you to **compare mass and width by running two different models**. To do this, you will need to run the model above, and then compare results to a model in which you replace `mass` with `width`. You can use DIC to compare the models initially.

I want you to standardize the covariates prior to including them in the model so we can compare the relative effect sizes of `width` and `mass` based on the magnitudes of `beta` estimated for each model.
...
For your results, it is fine to present model predictions on the standardized scale of the variable. I do want you to compare the relative effects of `mass` and `width` using a combination of DIC and the standardized coefficient estimates.
...
# Call jags and run the model
  crabs.model <- jags(crabs.data, inits=inits, parameters, 
    model.file = "crabsMod.txt",
    n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb,
    working.directory = getwd())

...
```{r}
# This is a comment. We know because it is preceded by a hashtag, or 'octothorpe'
```
...
3. Either 1 or 2, but press the `Run` button at the top of this source file to run the code block.
...
We can ask R what functions mean like this...and not just functions: we can ask it about pretty much any built-in object. **Remember that**.
...
```{r}
    ...
    !is.na(a)    # The exclamation point means 'not' and is a logical operator
    a == 3       # We can see if specific elements of a vector meet a criterion
...
This and the atomic vector, are probably the data structures that we will use most in this course.

...
\item 1. Use the degree day data from 2019 that we made (waaaay up top) to predict mean and 95% CI for counts from each of the models above, using any of the methods you have learned in this course. Treat these like sequences that we've been passing to the `newdata` argument in the `predict` function all semester, or that we have recently been using to make predictions by hand. **Note** that you will not get the back end of the curve because it is early April right now!
...
clim19$date[first(which(clim19$dd > init_l))]
clim19$date[first(which(clim19$dd > init_m))]
clim19$date[first(which(clim19$dd > init_u))]

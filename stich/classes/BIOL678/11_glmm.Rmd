...
Notice that alpha is a matrix with six columns? Need that!
```{r}
str(posts)
```
...
Now, we can actually look at the change in flow with respect to a given year and calculate the probability of using the Stillwater Branch given flow and some new values for beta.
...
Some of you may have noticed that the slope of lines does not change in this example. We could, of course implement this change in our model, and this essentially becomes the equivelant of adding an interaction term to our model (separate slopes and intercepts).
...
Notice that this one takes a little longer to run, but now we can essentially make the same predictions that we did before and the rest of our estimated parameters are basically identical to the values we got previously.

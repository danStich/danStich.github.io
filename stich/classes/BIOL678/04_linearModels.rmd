...
We find that there are significant shifts in median `Ozone` between `months`. Hopefully, it is now obvious that we can use a boxplot to visualize these differences.
```{r}
boxplot(Ozone ~ Month,
        data = airquality,
        col='gray',
...
We see that there is a significant negative (inverse) relation between `Education` level and `fertility`. In other words, more highly educated individuals have fewer children. You can tell this is a negative relationship because of the minus sign in front of the coefficient. We know that the relationship is significant because of the small p-value and corresponding significance codes.

...
If you haven't seen it yet, [here](https://www.youtube.com/watch?v=u1r5XTqrCTQ) is a three-minute video explaining how to install packages using RStudio.
...
We can use functions from the `dplyr` package to calculate mean `Length` of fish for each combindation of `Sex` and `Age` group much more easily than we did for a single group above.
